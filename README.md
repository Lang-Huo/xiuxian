...
| POST | `/api/spirit-root/start` | 开始**测灵根**（query: `difficulty` 可选，默认入门；固定 5 题） | 是 |
| POST | `/api/spirit-root/submit` | 提交 5 题答案，返回所得灵根与正确率（body: `answers: [{questionId, userAnswer}]`） | 是 |
...
- **我的**：**左右两栏**布局
  - **左栏（名帖与修为）**：顶部名帖（左侧大头像 = 昵称首字、右侧昵称/境界徽章/称号/仙途编号/修行天数，右上角「换头像」可切换 12 款内置头像）；下方「个人信息」（编号、手机号、境界、入道日期、道行、距下境界）与「修为进度」（境界条 + 气血条 + 进阶提示）；再往下是修炼战绩 5 宫格与最近 5 条修炼记录
  - **右栏（行囊 / 背包）**：储物空间进度（已用 / 总格数，标明境界基础与装备加成）、4 个装备槽、行囊格子网格（空格一眼可见）；点格子看详情，可装备 / 卸下 / 服用 / 丢弃

> 切到「我的」时名帖与行囊一并重新拉取；在行囊中服用丹药会同步刷新顶部气血。
>
> 页面顺序为「页头 → 主导航 → 内容」；道行 / 气血 / 答题统计那一栏**只在「修炼」页内显示**，切到「我的」会自动收起。
>
> 「我的」页容器放宽到 1080px 以容纳两栏；窄屏（≤900px）自动堆叠为单列。
...
- **默认 = 未测灵根**：道行倍率 1.0（无加成也无损失）。注册成功后前端自动弹出测灵根引导，可点「跳过」进入「修炼」
- **测灵根流程**：用户选难度 → 后端按该难度出 5 题 → 一次性提交答案 → 按正确率判定灵根并入库
- 判定阈值（5 题）：
  ...
  | ≥ 60% | 单灵根 | 较稀有 | ×1.15 |
  | ≥ 80% | 变异灵根 | 稀有 | ×1.3 |
  | 100% | 天灵根（85%）/ 混沌灵根（15%） | 稀有 / 极稀有 | ×1.5 / ×2.0 |

- 规则集中在 `com.xiuxian.service.SpiritRootPolicy`，调阈值/倍率只改一处
- `GameService` 答题结算时自动乘以灵根倍率：`expGain = BASE_EXP × 难度 × (1+装备加成%) × 灵根倍率`
- 测灵根出题复用 `QuestionService.generate`，但主题关键字带 `【测灵根】` 前缀，`TopicMapper` 在「修炼主题数 / 最近修炼」两条 SQL 里排除该前缀，避免测试记录污染修炼历程
- 判题逻辑提取为 `com.xiuxian.util.AnswerUtil`，答题结算与测灵根共用同一规则
- 「我的」页：已测显示灵根卡（名 / 稀有度 / 倍率 / 描述）；未测时显示「灵根未定」+「立即测灵根」入口（调起引导弹窗）

### 答题结算（道行与气血）

| 情形 | 道行 | 气血 |
|------|------|------|
| 答对 | 基础 10 × 难度系数 × (1 + 装备加成%) | **+3**（不超过上限，已满则不回） |
| 答错 | 上述 × 0.3（悟道分） | **只扣不回**：入门 −8 / 进阶 −12 / 精通 −16 |

- 答错**绝不回血**；当剩余气血不足以支付本次伤害时，**直接归零**，不会出现负值
- 归零后 `canAnswer = false`，无法继续答题，需在「我的 → 行囊」服用丹药回复
- 返回的 `hpDelta` 是**真实变化量**（满血答对时为 0，而不是显示 +3）
